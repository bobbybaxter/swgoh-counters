...
module.exports = ( { data, patreon } ) => ( {
  ...
  handler: async ( request, reply ) => {
    let accessToken, patron, patreonId, patreonEmail, refreshToken;
    const { code } = request.query;
    const redirect = process.env.REDIRECT_URL;
    const myCampaignId = '3137849';
    const now = new Date();
    const expireDate = new Date();
    const expiresIn = new Date( expireDate.setDate( expireDate.getDate() + 30 ));
    ...
    const patronInfo = await apiClient( {
      method: 'GET',
    ...
    if ( patronInfo && patronInfo.rawJson && patronInfo.rawJson.included ) {
      ...
      if ( membership ) {
        ...
        if ( user.expiresIn && user.expiresIn < now.toISOString()) {
          ...
          ( { accessToken } = refreshedPatronToken );
          ( { refreshToken } = refreshedPatronToken );
        }
